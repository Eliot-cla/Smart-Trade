...
export default function SmartTrade() {
  ...
  const renderSidebar = (active) => {
    ...
    return (
      <div className="sidebar">
        ...
        <div style={{ flex: 1 }} />
        <div style={{ borderTop: `1px solid ${LINE}`, margin: "16px 0 8px", paddingTop: 12, display: "flex", flexDirection: "column", gap: 2 }}>
          <button onClick={() => setView("terms")} className={`sidebar-link mono${active === "terms" ? " active" : ""}`} style={{ fontSize: 11.5, color: "#5A564E" }}>
            <FileText size={13} />
            Terms
          </button>
          <button onClick={() => setView("privacy")} className={`sidebar-link mono${active === "privacy" ? " active" : ""}`} style={{ fontSize: 11.5, color: "#5A564E" }}>
            <FileText size={13} />
            Privacy
          </button>
        </div>
        {session && (
          <button onClick={logOut} className="sidebar-link mono" style={{ marginTop: 8 }}>
            <LogOut size={15} />
            Log out
          </button>
        )}
  ...
  if (view === "pricing") {
    const plans = [
      { id: "pack", priceId: "price_1U7hIUAj5N9sH4585K2CCCFi", name: "Starter Pack", price: "9.99 €", period: "one-time", tag: null, desc: "20 chart analyses, no expiry.", features: ["20 analyses", "Full structure breakdown", "Market context toggle"] },
      { id: "pack2", priceId: "price_1U7hIUAj5N9sH458epm9fMNv", name: "Growth Pack", price: "24.99 €", period: "/ month", tag: null, desc: "75 chart analyses every month.", features: ["75 analyses / month", "Full structure breakdown", "Market context toggle"] },
      { id: "monthly", priceId: "price_1U7hIUAj5N9sH458QQSy81r3", name: "Unlimited Monthly", price: "39.99 €", period: "/ month", tag: "Most popular", desc: "Unlimited analyses, cancel anytime.", features: ["Unlimited analyses", "Full structure breakdown", "Market context toggle", "Priority processing"] },
  ...
  // ---------- TERMS OF SERVICE ----------
  // Reachable only via the sidebar — deliberately not part of the
  // welcome → landing → auth → pricing → app sequence.
  if (view === "terms") {
    return (
      <div style={{ minHeight: "100vh", background: INK, color: TEXT, fontFamily: "'Inter', sans-serif", position: "relative", overflow: "hidden" }}>
        {sharedStyles}
        <div className="app-shell">
          {renderSidebar("terms")}
          <div className="main-content">
            <div style={{ maxWidth: 680, margin: "0 auto", padding: "48px 20px 90px" }}>
              <button onClick={() => setView("app")} className="btn mono" style={{ display: "flex", alignItems: "center", gap: 6, background: "transparent", border: "none", color: MUTE, fontSize: 12.5, padding: 0, marginBottom: 22 }}>
                <ArrowLeft size={14} /> Back
              </button>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: GOLD }} />
                <span className="mono" style={{ fontSize: 11, letterSpacing: 3, color: GOLD, textTransform: "uppercase" }}>Legal</span>
              </div>
              <h1 className="disp" style={{ fontSize: 32, fontWeight: 480, margin: "6px 0 6px", letterSpacing: -0.3 }}>Terms of Service</h1>
              <p className="mono" style={{ fontSize: 12, color: "#5A564E", marginBottom: 32 }}>Last updated: August 2026</p>

              <div className="mono" style={{ fontSize: 13.5, lineHeight: 1.75, color: TEXT }}>
                {[
                  ["1. Acceptance of these terms", "By creating an account or using Smart Trade (\"the Service\"), you agree to these Terms of Service. If you don't agree, please don't use the Service."],
                  ["2. What Smart Trade is", "Smart Trade reads chart screenshots you upload and produces an automated, pattern-based technical summary — a possible direction, indicative entry/take-profit/stop-loss levels, and related context. This is generated by an AI model reading visual patterns. It is not financial, investment, or trading advice, not a recommendation to buy or sell any asset, and not a guarantee of any outcome. You are solely responsible for any trading or investment decisions you make, and trading carries a real risk of financial loss."],
                  ["3. Your account", "You're responsible for keeping your login credentials secure and for all activity under your account. Let us know if you believe your account has been compromised."],
                  ["4. Subscriptions and billing", "Where paid plans are offered, prices and included usage are shown at checkout. Payment processing (once enabled) will be handled by a third-party payment provider; we don't store your full card details ourselves. Subscriptions may be cancelled at any time; unless stated otherwise, fees already paid are non-refundable for the current billing period."],
                  ["5. Acceptable use", "Don't use the Service to break the law, to attempt to disrupt or reverse-engineer it, to resell or redistribute its output as your own automated advisory product without permission, or to abuse shared infrastructure (e.g. excessive automated requests)."],
                  ["6. No financial advice", "Nothing in the Service constitutes financial, legal, or tax advice. Outputs are automated interpretations of visual data and may be inaccurate, incomplete, or wrong. Always do your own research and consider consulting a licensed professional before making financial decisions."],
                  ["7. Limitation of liability", "The Service is provided \"as is,\" without warranties of any kind. To the fullest extent permitted by law, Smart Trade and its operator are not liable for any trading losses, financial damages, or other losses arising from your use of the Service."],
                  ...
                  <div key={i} style={{ marginBottom: 22 }}>
                    ...
                    <p style={{ margin: 0, color: MUTE }}>{body}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="app-shell">
          ...
          <div className="main-content">
            <div style={{ maxWidth: 680, margin: "0 auto", padding: "48px 20px 90px" }}>
              ...
              <h1 className="disp" style={{ fontSize: 32, fontWeight: 480, margin: "6px 0 6px", letterSpacing: -0.3 }}>Privacy Policy</h1>
              <p className="mono" style={{ fontSize: 12, color: "#5A564E", marginBottom: 32 }}>Last updated: August 2026</p>

              <div className="mono" style={{ fontSize: 13.5, lineHeight: 1.75, color: TEXT }}>
                {[
                  ["1. What we collect", "Account data: your email address and password (stored securely, hashed — we never see it in plain text). Content you provide: chart screenshots you upload for analysis, and any analyses you choose to save to your Track Record (direction, levels, thumbnail, and your self-reported outcomes)."],
                  ["2. How we use it", "To provide the Service: authenticate you, run your chart analysis, and store your Track Record so it's there next time you sign in. We don't use your data to train AI models, and we don't sell your personal data to third parties."],
                  ["3. Who processes your data", "We rely on a small number of trusted infrastructure providers to run the Service: a database and authentication provider to store your account and Track Record, an AI provider to analyze the chart images you upload and generate the technical read, and a hosting provider to serve the app itself. Each only receives the data necessary to perform its function."],
                  ["4. Chart images", "Images you upload are sent to our AI provider for analysis. A small thumbnail may be stored with a saved Track Record entry; we don't use your images for any purpose beyond generating your analysis."],
                  ["5. Data retention", "Your account and Track Record data are kept until you delete individual entries, delete your account, or request removal. You can delete saved analyses at any time from Track Record."],
                  ["6. Your rights", "You can access, correct, or delete your account data at any time from Settings. If you're in the EU/EEA or Switzerland, you have rights under GDPR/FADP including access, correction, deletion, and data portability — contact us to exercise these if you can't do so directly in the app."],
